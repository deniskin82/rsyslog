...
#	include <sys/inotify.h>	/* use inotify if availabe to wait for writes to log files */
#endif
...
typedef struct fileInfo_s {
	...
	int wd;		/* the inotify watch descriptor for pszFileName or 0 if none */
	uchar *pszWatchedFileName; /* the path of the file opened by pStrm or NULL if none */
} fileInfo_t;
...
/* the inotify fd */
static int fdInotify = -1;

/* Input buffer when we read() from the inotify fd. Since we use
 * IN_ONESHOT there will be at most one event per file. We want to
 * read all events at once, so make room for MAX_INPUT_FILES entries
 * with a generous 128 char path each.
 *
 * Note that IN_ONESHOT only works properly on Linux kernels 2.6.16
 * and onwards, but in practice that shouldn't be a big problem. If
 * pInotifyBuf is not big enough for all events we'll just do some
 * unnecessry pollFile() calls, but that's OK. It is also OK if the
 * inotify kernel buffer overflows (IN_Q_OVERFLOW), because we don't
 * care much about *what* events that have occured, only *if* they
 * occur.
 */
static char pInotifyBuf[MAX_INPUT_FILES * (sizeof(struct inotify_event) + 128)];
...
static rsRetVal
addWatch(fileInfo_t *pInfo)
{
	DEFiRet;
#ifdef HAVE_SYS_INOTIFY_H
	const uchar *pWatchedFile = pInfo->pszWatchedFileName;

	if (pWatchedFile == NULL) {
		ABORT_FINALIZE(RS_RET_OK);
	}

	/* it's OK if the watch exists already; we'll get the old one */
	pInfo->wd = inotify_add_watch(fdInotify, (char*)pWatchedFile, IN_MODIFY | IN_ONESHOT);
	if (pInfo->wd == -1) {
		DBGPRINTF("could not inotify_add_watch() on '%s': %s\n", pWatchedFile, strerror(errno));
	}
...
/* callback from strm_t when a file is opened. We use it to setup an
 * inotify watch on the file so we can get notified when the file is
 * written to
 */
static rsRetVal
onFileOpened(strm_t __attribute__((unused)) *psSF,
	     const uchar __attribute__((unused)) *pszName,
	     int __attribute__((unused)) iFlags,
	     mode_t __attribute__((unused)) tMode,
	     void __attribute__((unused)) *pData)
{
	DEFiRet;
#ifdef HAVE_SYS_INOTIFY_H
	fileInfo_t *pInfo = (fileInfo_t *)pData;
	free (pInfo->pszWatchedFileName);
	pInfo->pszWatchedFileName = ustrdup(pszName);
	if (pInfo->pszWatchedFileName == NULL) {
		ABORT_FINALIZE(RS_RET_OUT_OF_MEMORY);
	}
	DBGPRINTF("added '%s' to inotify watch list\n", (char*)pInfo->pszWatchedFileName);

	/* it is not enough to add the watch in prepareForReading()
	 * because it will get called before the first onFileOpened
	 */
	addWatch(pInfo);
finalize_it:
#endif /* #ifdef HAVE_SYS_INOTIFY_H */
...
/* called by strm_t when a file is closed. We use it to remove any
 * inotify watch
 */
static rsRetVal
onFileClosed(strm_t __attribute__((unused)) *psSF,
	     const uchar __attribute__((unused)) *pszName,
	     void __attribute__((unused)) *pData)
{
	DEFiRet;
#ifdef HAVE_SYS_INOTIFY_H
	fileInfo_t *pInfo = (fileInfo_t *)pData;
	DBGPRINTF("removing '%s' from inotify watch list\n", (char*)pInfo->pszWatchedFileName);
	free (pInfo->pszWatchedFileName);
	pInfo->pszWatchedFileName = NULL;
	if (inotify_rm_watch(fdInotify, pInfo->wd) == -1) {
		if (errno == EBADF) {
			ABORT_FINALIZE(RS_RET_INVALID_PARAMS);
		} else if (errno == EINVAL) {
			/* probably because of automatic removal by IN_ONESHOT, ignore */
		}
	}
finalize_it:
#endif /* #ifdef HAVE_SYS_INOTIFY_H */
...
/* If inotify is available, make sure all files are watched. We use
 * IN_ONESHOT to prevent the event queue from flooding if there are
 * lots of writes to files while we are reading
 */
static rsRetVal
prepareForReading()
{
...
#ifdef HAVE_SYS_INOTIFY_H
	if (fdInotify == -1) {
		/* we earlier failed to initialize inotify */
...
/* Wait for any file to be written to. If inotify is available, use
 * inotify to listen for writes to files. If not, use sleep,
 * effectively causing us to poll files.
 */
static rsRetVal
waitForChanges()
{
	...
	while (glbl.GetGlobalInputTermState() == 0) {
		/* this blocks until there are IN_MODIFY events i.e. until one
		 * of the files we watch is write()ten to (or until there is
		 * an error)
		 *
		 * XXX: Do we need to check glbl.GetGlobalInputTermState()
		 * every now and then?
		 */
		int n = read(fdInotify, pInotifyBuf, sizeof(pInotifyBuf));

		if (n == -1 && errno == EINTR) {
			/* we were just interupted, continue waiting for events */
			continue;
		} else if (n == -1 || n == 0) {
			/* something went wrong. note that we don't expect n == 0
			 * on an inotify fd
			 */
			DBGPRINTF("filemon waitForChanges(): read() failed: %s\n", strerror(errno));
			/* break so that we don't risk an infinite loop */
			break;
		} else {
			/* assume we successfully read all events and
			 * get out of this loop since there are
			 * changes to a file (we don't care if we
			 * really got IN_IGNORED or IN_Q_OVERFLOW
			 * events, those should be rare anyway). If we
			 * didn't read all events, it's not a big
			 * deal, we'll just not block here next time,
			 * but some extra file reading doedsn't hurt.
			 */
			DBGPRINTF("inotify reported changes to a file!\n");
			break;
		}
	}
#else
...
static rsRetVal
setupStreamCallbacks(fileInfo_t *pInfo) {
	DEFiRet;
	DBGPRINTF("setting up 'onFileOpened' stream callback for %s!\n", pInfo->pszFileName);
	CHKiRet(strm.SetOnFileOpened(pInfo->pStrm, onFileOpened, pInfo));
	CHKiRet(strm.SetOnFileClosed(pInfo->pStrm, onFileClosed, pInfo));
finalize_it:
	RETiRet;
}
...
openFile(fileInfo_t *pThis)
{
	...
	/* we must set up these callbacks before we seek */
	CHKiRet(setupStreamCallbacks(pThis));
...
CODESTARTrunInput
	...
	while(glbl.GetGlobalInputTermState() == 0) {
		/* do necessary preperations before reading */
		prepareForReading();
		...
		/* wait for changes to any of the files we monitor */
		waitForChanges();
	}
...
CODEmodInit_QueryRegCFSLineHdlr
	...
	if (fdInotify == -1) {
		dbgprintf("filemon prepareForReading(): inotify_init() failed: %s\n", strerror(errno));
		ABORT_FINALIZE(RS_RET_IO_ERROR);
	}
#endif /* #ifdef HAVE_SYS_INOTIFY_H */
